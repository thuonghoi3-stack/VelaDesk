...
const url = process.env.VELA_QA_URL || 'http://127.0.0.1:8093/';
const mode = 'integration-production';
const out = `.hermes/artifacts/logic-repair/browser-${Date.now()}`;
...
try {
  ...
  const assets=await page.locator('script[src]').evaluateAll(xs=>xs.map(x=>x.src));
  assert.ok(assets.some(x=>x.includes('/assets/')));
  result.productionAssets=[];
  for(const asset of assets.filter(x=>x.startsWith(new URL(url).origin))){const response=await page.request.get(asset);assert.equal(response.status(),200);result.productionAssets.push({url:asset,status:response.status(),bytes:(await response.body()).length});}
  ...
  // Actual UI steps, including final candle: no store imports or synthetic state injection.
  const beforeEnd=await state();
  for(let i=0;i<Math.ceil(total/10);i++){
    if(await page.getByRole('button',{name:'Bước 10 nến',exact:true}).isDisabled())break;
    await click('Bước 10 nến');
  }
  const terminal=await state();
  assert.equal(terminal.processed,total);assert.equal(terminal.index,6000);assert.equal(terminal.progress,100);
  assert.ok(await page.getByTestId('replay-play').isDisabled());
  assert.ok(await page.getByRole('button',{name:'Bước 1 nến',exact:true}).isDisabled());
  assert.equal(terminal.time,start.time+total*(one.time-start.time));
  assert.ok((await page.getByTestId('replay-panel').innerText()).includes('Đã hoàn tất'));
  result.checks.push({name:'Full history final candle completes without phantom n+1',terminal});
  result.terminalText=await page.getByTestId('replay-panel').innerText();
  result.terminalAccount={cash:await page.getByTestId('replay-cash').innerText(),mtm:await page.getByTestId('replay-mtm').innerText()};
  assert.equal(result.terminalAccount.cash,result.terminalAccount.mtm);
  assert.ok(result.terminalText.includes('Không có vị thế'));
  await page.screenshot({path:`${out}/terminal.png`,fullPage:true});
  await click('Đặt lại replay');assert.deepEqual(await state(),start);
  await click('Tester');
  assert.ok(await page.locator('input[type=date]').nth(0).isDisabled());
  await click('Xem kết quả hiện tại');
  assert.ok(await page.locator('input[type=date]').nth(0).isEnabled());
  const from=new Date(start.time).toISOString().slice(0,10),to=new Date(terminal.time).toISOString().slice(0,10);
  await page.locator('input[type=date]').nth(0).fill(from);
  await page.locator('input[type=date]').nth(1).fill(to);
  result.range={from,to};
  const buttons=await page.getByRole('button').allTextContents();result.rangeButtons=buttons;
  await click('Chạy lại');
  await click('Danh sách lệnh');
  await page.getByTestId('trade-row').first().getByRole('button').click();
  const rangeEntry=Number(await page.getByTestId('trade-detail').getAttribute('data-entry-time'));
  await click('Replay từ điểm vào');await page.getByTestId('replay-historical-context').waitFor();
  const rangeStart=await state();assert.equal(rangeStart.processed,0);assert.ok(rangeStart.time<rangeEntry);
  await click('Bước 1 nến');assert.equal((await state()).time,rangeEntry);
  await click('Đặt lại replay');assert.deepEqual(await state(),rangeStart);
  result.checks.push({name:'Date range historical handoff and reset',rangeEntry,rangeStart});
  await page.screenshot({path:`${out}/range.png`,fullPage:true});
